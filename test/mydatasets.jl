...
datasets_info = [
    ...
    #     to_exlude = [:price_range]),

    # (name_str = "yeast.csv",
    #     name_symbol = :Y,
    #     target_c = :localization_site,
    #     to_exlude = [:localization_site, :Sequence_Name]),
...
mydatasets =  map(di -> begin

    datatable = CSV.read((joinpath(module_dir, di.name_str)), DataFrame)
    # datatable = CSV.read((joinpath(module_dir, "test", di.name_str)), DataFrame)
    y = datatable[:, di.target_c] |> CategoricalArray
    X = DataFrames.select(datatable, Not(di.to_exlude));

        X, y = preprocess_inputdata(X,y)
        (di.name_symbol => (X, y))

end, datasets_info) |> OrderedDict
